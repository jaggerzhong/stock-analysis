...
class StockData:
    ...
    shares_outstanding: Optional[float] = None  # Total shares outstanding (for DCF per-share calculation)
...
class ValuationMetricsCalculator:
    ...
    def calculate_graham_intrinsic_value(eps: float, growth_rate: float, bond_yield: float) -> Optional[float]:
        """
        Calculate Graham intrinsic value (REFERENCE ONLY — not for composite valuation)

        Graham intended this formula for growth rates 0-15%. For higher growth rates,
        the result becomes wildly optimistic. We cap at 15% to prevent inflation.

        The 透支预期 (overstretch %) metric uses this as a reference anchor only.

        Args:
            eps: Earnings per share
            growth_rate: Expected growth rate (decimal), capped at 0.15
            bond_yield: Current AAA corporate bond yield (decimal)

        Returns:
            intrinsic_value: Graham intrinsic value (reference only)
        """
        if bond_yield <= 0:
            return None

        capped_growth = min(growth_rate, 0.15)
        base_value = eps * (8.5 + 2 * capped_growth * 100)
    ...
    def calculate_valuation_score(valuation_metrics: Dict[str, float], sector: Optional[str] = None) -> Tuple[float, Dict]:
        ...
        # Shiller P/E margin of safety scoring
        shiller_mos = valuation_metrics.get('shiller_margin_of_safety')
        if shiller_mos is not None:
            if shiller_mos > 0.30:
                scores['shiller'] = 10
            elif shiller_mos > 0.15:
                scores['shiller'] = 25
            elif shiller_mos > 0:
                scores['shiller'] = 45
            elif shiller_mos > -0.10:
                scores['shiller'] = 60
            elif shiller_mos > -0.25:
                scores['shiller'] = 75
            else:
                scores['shiller'] = 90
    ...
    @staticmethod
    def calculate_dcf_intrinsic_value(current_fcf: float, shares_outstanding: float,
                                       growth_rate_high: float = 0.12, years_high: int = 5,
                                       growth_rate_transition: float = 0.06, years_transition: int = 5,
                                       terminal_growth_rate: float = 0.025, wacc: float = 0.10,
                                       net_debt: float = 0) -> Optional[Dict]:
        """
        Calculate DCF intrinsic value with three scenarios (conservative/base/optimistic).

        Three-stage DCF model:
        1. High growth phase (years 1-5): growth_rate_high
        2. Transition phase (years 6-10): growth decelerating to terminal
        3. Terminal value: Gordon Growth Model with terminal_growth_rate

        Args:
            current_fcf: Current annual free cash flow (total, not per share)
            shares_outstanding: Total shares outstanding
            growth_rate_high: Initial high growth rate (decimal, default 0.12)
            years_high: Years of high growth (default 5)
            growth_rate_transition: Transition growth rate (decimal, default 0.06)
            years_transition: Years of transition growth (default 5)
            terminal_growth_rate: Long-term terminal growth rate (decimal, default 0.025)
            wacc: Weighted average cost of capital (decimal, default 0.10)
            net_debt: Net debt (total debt - cash)

        Returns:
            Dict with dcf_value (base), dcf_upper (optimistic), dcf_lower (conservative)
        """
        if current_fcf <= 0 or shares_outstanding <= 0 or wacc <= 0:
            return None

        def _dcf(fcf, g_high, g_trans, g_term, r):
            pv_high = 0.0
            pv_transition = 0.0
            cf = fcf
            ...
            for year in range(1, years_transition + 1):
                blend = year / years_transition
                g = g_high * (1 - blend) + g_term * blend
                terminal_cf *= (1 + g)
                pv_transition += terminal_cf / (1 + r) ** (years_high + year)
            terminal_value = terminal_cf * (1 + g_term) / (r - g_term)
            pv_terminal = terminal_value / (1 + r) ** (years_high + years_transition)
            total_pv = pv_high + pv_transition + pv_terminal
            equity_value = total_pv - net_debt
            per_share = equity_value / shares_outstanding
            return max(per_share, 0)

        base = _dcf(current_fcf, growth_rate_high, growth_rate_transition, terminal_growth_rate, wacc)
        optimistic = _dcf(current_fcf, growth_rate_high * 1.15, growth_rate_transition * 1.1,
                          terminal_growth_rate, wacc * 0.95)
        conservative = _dcf(current_fcf, growth_rate_high * 0.85, growth_rate_transition * 0.9,
                            terminal_growth_rate, wacc * 1.05)

        return {
            'dcf_value': round(base, 2),
            'dcf_upper': round(optimistic, 2),
            'dcf_lower': round(conservative, 2),
            'wacc_used': wacc,
            'growth_high_used': growth_rate_high,
            'growth_transition_used': growth_rate_transition,
            'terminal_growth_used': terminal_growth_rate,
        }

    @staticmethod
    def calculate_shiller_pe_intrinsic_value(current_eps: float, historical_eps_list: List[float],
                                              current_price: float,
                                              fair_cape_range: Tuple[float, float] = (15.0, 20.0)) -> Optional[Dict]:
        """
        Calculate Shiller P/E (CAPE) intrinsic value.

        Uses 10-year normalized earnings to smooth out business cycles.
        Fair CAPE range determines conservative/optimistic bounds.

        Args:
            current_eps: Current earnings per share
            historical_eps_list: List of EPS values (ideally 10 years)
            current_price: Current stock price
            fair_cape_range: (lower, upper) fair CAPE range

        Returns:
            Dict with shiller_pe, shiller_intrinsic_lower/mid/upper, cape_percentile
        """
        if not historical_eps_list or len(historical_eps_list) < 3 or current_eps <= 0:
            return None

        cape_lower, cape_upper = fair_cape_range
        cape_mid = (cape_lower + cape_upper) / 2

        normalized_eps = sum(historical_eps_list) / len(historical_eps_list)

        inflation_adjusted_eps = normalized_eps * 1.0
        if len(historical_eps_list) >= 5:
            recent_avg = sum(historical_eps_list[-3:]) / 3
            inflation_adjusted_eps = (normalized_eps + recent_avg) / 2

        shiller_pe = current_price / inflation_adjusted_eps if inflation_adjusted_eps > 0 else None

        intrinsic_lower = inflation_adjusted_eps * cape_lower
        intrinsic_mid = inflation_adjusted_eps * cape_mid
        intrinsic_upper = inflation_adjusted_eps * cape_upper

        if shiller_pe is not None:
            cape_range = cape_upper - cape_lower
            if cape_range > 0:
                cape_percentile = min(1.0, max(0.0, (shiller_pe - cape_lower) / cape_range)) * 100
            else:
                cape_percentile = 50.0
        else:
            cape_percentile = None

        return {
            'shiller_pe': round(shiller_pe, 2) if shiller_pe else None,
            'shiller_intrinsic_lower': round(intrinsic_lower, 2),
            'shiller_intrinsic_mid': round(intrinsic_mid, 2),
            'shiller_intrinsic_upper': round(intrinsic_upper, 2),
            'normalized_eps': round(inflation_adjusted_eps, 4),
            'cape_percentile': round(cape_percentile, 1) if cape_percentile is not None else None,
        }

    @staticmethod
    def calculate_valuation_range(dcf_result: Optional[Dict] = None,
                                    shiller_result: Optional[Dict] = None,
                                    graham_value: Optional[float] = None,
                                    current_price: float = 0) -> Dict:
        """
        Calculate composite valuation range using DCF (55%) + Shiller P/E (45%).

        Graham intrinsic value is reference-only (透支预期), NOT part of the composite.

        Args:
            dcf_result: Output from calculate_dcf_intrinsic_value()
            shiller_result: Output from calculate_shiller_pe_intrinsic_value()
            graham_value: Graham intrinsic value (reference only)
            current_price: Current stock price

        Returns:
            Dict with valuation_lower, valuation_mid, valuation_upper,
            margin_of_safety, price_vs_range, and graham_reference
        """
        DCF_WEIGHT = 0.55
        SHILLER_WEIGHT = 0.45

        dcf_available = dcf_result is not None and dcf_result.get('dcf_value') is not None
        shiller_available = shiller_result is not None and shiller_result.get('shiller_intrinsic_mid') is not None
        ...
        if dcf_available:
            dcf_lower = dcf_result['dcf_lower']
            dcf_mid = dcf_result['dcf_value']
            dcf_upper = dcf_result['dcf_upper']
            lower_components['dcf'] = dcf_lower * DCF_WEIGHT
            mid_components['dcf'] = dcf_mid * DCF_WEIGHT
            upper_components['dcf'] = dcf_upper * DCF_WEIGHT
            total_weight += DCF_WEIGHT

        if shiller_available:
            shiller_lower = shiller_result['shiller_intrinsic_lower']
            shiller_mid = shiller_result['shiller_intrinsic_mid']
            shiller_upper = shiller_result['shiller_intrinsic_upper']
            w = SHILLER_WEIGHT if total_weight == 0 else SHILLER_WEIGHT / (total_weight + SHILLER_WEIGHT) * (total_weight + SHILLER_WEIGHT)
            if dcf_available:
                w = SHILLER_WEIGHT
            else:
                w = 1.0
            lower_components['shiller'] = shiller_lower * w
            mid_components['shiller'] = shiller_mid * w
            upper_components['shiller'] = shiller_upper * w
            total_weight += w if not dcf_available else SHILLER_WEIGHT

        if total_weight == 0:
            return {
                'valuation_lower': None,
                'valuation_mid': None,
                'valuation_upper': None,
                'margin_of_safety': None,
                'price_vs_range': 'No data',
                'models': {},
                'graham_reference': None,
            }

        valuation_lower = sum(lower_components.values()) / total_weight
        valuation_mid = sum(mid_components.values()) / total_weight
        valuation_upper = sum(upper_components.values()) / total_weight

        if current_price > 0 and valuation_mid > 0:
            margin_of_safety = (valuation_mid - current_price) / valuation_mid
        else:
            margin_of_safety = None

        if current_price > 0 and valuation_lower is not None and valuation_upper is not None:
            if current_price < valuation_lower:
                price_vs_range = 'Below Range'
            elif current_price < valuation_mid:
                price_vs_range = 'Within Range (Undervalued)'
            elif current_price < valuation_upper:
                price_vs_range = 'Within Range (Overvalued)'
            else:
                price_vs_range = 'Above Range'
        else:
            price_vs_range = 'Unknown'

        graham_reference = None
        if graham_value is not None and current_price > 0 and graham_value > 0:
            stretch_pct = (current_price - graham_value) / graham_value
            if stretch_pct < -0.20:
                stretch_status = '远低于Graham价值'
            elif stretch_pct < 0:
                stretch_status = '低于Graham价值'
            elif stretch_pct < 0.20:
                stretch_status = '接近Graham价值'
            elif stretch_pct < 0.50:
                stretch_status = '透支Graham价值'
            else:
                stretch_status = '严重透支Graham价值'
            graham_reference = {
                'value': round(graham_value, 2),
                'stretch_pct': round(stretch_pct, 4),
                'stretch_status': stretch_status,
            }

        return {
            'valuation_lower': round(valuation_lower, 2) if valuation_lower else None,
            'valuation_mid': round(valuation_mid, 2) if valuation_mid else None,
            'valuation_upper': round(valuation_upper, 2) if valuation_upper else None,
            'margin_of_safety': round(margin_of_safety, 4) if margin_of_safety is not None else None,
            'price_vs_range': price_vs_range,
            'models': {
                'dcf': dcf_result if dcf_available else None,
                'shiller': shiller_result if shiller_available else None,
            },
            'graham_reference': graham_reference,
            'weights': {'dcf': DCF_WEIGHT, 'shiller': SHILLER_WEIGHT} if dcf_available and shiller_available else {'available': total_weight},
        }

    @staticmethod
    def analyze_stock_valuation(stock_data: StockData, bond_yield: float = 0.055) -> Dict:
        """
        ...
        # Calculate Graham intrinsic value (REFERENCE ONLY — not in composite)
        if stock_data.eps and stock_data.estimated_growth:
            ...
            analysis['graham_is_reference_only'] = True
            ...
            if graham_value and stock_data.current_price:
                ...
                stretch_pct = (stock_data.current_price - graham_value) / graham_value
                if stretch_pct < -0.20:
                    analysis['graham_stretch_status'] = '远低于Graham价值'
                elif stretch_pct < 0:
                    analysis['graham_stretch_status'] = '低于Graham价值'
                elif stretch_pct < 0.20:
                    analysis['graham_stretch_status'] = '接近Graham价值'
                elif stretch_pct < 0.50:
                    analysis['graham_stretch_status'] = '透支Graham价值'
                else:
                    analysis['graham_stretch_status'] = '严重透支Graham价值'
                analysis['graham_stretch_pct'] = round(stretch_pct, 4)

        # Calculate DCF intrinsic value (CORE — 55% of composite)
        if (stock_data.free_cash_flow and stock_data.free_cash_flow > 0 and
            getattr(stock_data, 'shares_outstanding', None) and getattr(stock_data, 'shares_outstanding', None) > 0):
            fcf = stock_data.free_cash_flow
            shares = getattr(stock_data, 'shares_outstanding', None)
            growth = stock_data.estimated_growth or 0.08
            growth_capped = min(growth, 0.20)

            wacc_params = get_config('value_trap', 'wacc_params', default={
                'risk_free_rate': 0.05, 'market_risk_premium': 0.06,
                'default_cost_of_debt': 0.055, 'default_tax_rate': 0.25
            })
            risk_free = wacc_params.get('risk_free_rate', 0.05)
            market_prem = wacc_params.get('market_risk_premium', 0.06)
            beta = getattr(stock_data, 'beta', None) or 1.0
            cost_of_equity = risk_free + beta * market_prem
            equity = stock_data.shareholders_equity or 1
            debt = stock_data.total_debt or 0
            total_cap = equity + debt if (equity + debt) > 0 else 1
            equity_w = equity / total_cap
            debt_w = 1 - equity_w
            cost_debt = wacc_params.get('default_cost_of_debt', 0.055)
            tax_rate = wacc_params.get('default_tax_rate', 0.25)
            wacc = cost_of_equity * equity_w + cost_debt * (1 - tax_rate) * debt_w
            wacc = max(wacc, 0.06)

            net_debt = (stock_data.total_debt or 0) - (stock_data.cash or 0)

            dcf_result = ValuationMetricsCalculator.calculate_dcf_intrinsic_value(
                current_fcf=fcf, shares_outstanding=shares,
                growth_rate_high=growth_capped, years_high=5,
                growth_rate_transition=growth_capped * 0.5, years_transition=5,
                terminal_growth_rate=0.025, wacc=wacc, net_debt=net_debt
            )
            analysis['dcf_result'] = dcf_result
            if dcf_result and dcf_result.get('dcf_value'):
                dcf_mos = (dcf_result['dcf_value'] - stock_data.current_price) / dcf_result['dcf_value']
                analysis['dcf_margin_of_safety'] = dcf_mos

        # Calculate Shiller P/E intrinsic value (CORE — 45% of composite)
        sector_benchmarks = _get_sector_pe_benchmarks()
        sector = stock_data.sector or 'Technology'
        sector_data = sector_benchmarks.get(sector, sector_benchmarks.get(sector.replace('_', ' '), {}))
        fair_cape_lower = sector_data.get('cheap', 15) if sector_data else 15
        fair_cape_upper = sector_data.get('expensive', 25) if sector_data else 25
        fair_cape_range = (float(fair_cape_lower), float(fair_cape_upper))

        shiller_result = None
        if stock_data.eps and stock_data.current_price and stock_data.eps > 0:
            normalized_eps = stock_data.eps
            shiller_result = ValuationMetricsCalculator.calculate_shiller_pe_intrinsic_value(
                current_eps=stock_data.eps,
                historical_eps_list=[stock_data.eps],
                current_price=stock_data.current_price,
                fair_cape_range=fair_cape_range
            )
            analysis['shiller_result'] = shiller_result
            if shiller_result and shiller_result.get('shiller_intrinsic_mid'):
                shiller_mos = (shiller_result['shiller_intrinsic_mid'] - stock_data.current_price) / shiller_result['shiller_intrinsic_mid']
                analysis['shiller_margin_of_safety'] = shiller_mos

        # Calculate valuation_range (DCF 55% + Shiller 45%, Graham reference-only)
        valuation_range = ValuationMetricsCalculator.calculate_valuation_range(
            dcf_result=analysis.get('dcf_result'),
            shiller_result=analysis.get('shiller_result'),
            graham_value=analysis.get('graham_intrinsic_value'),
            current_price=stock_data.current_price or 0
        )
        analysis['valuation_range'] = valuation_range

        # Add DCF and Shiller margins to valuation metrics for score calculation
        if 'dcf_margin_of_safety' in analysis:
            analysis['dcf_margin_of_safety'] = analysis['dcf_margin_of_safety']
        if 'shiller_margin_of_safety' in analysis:
            analysis['shiller_margin_of_safety'] = analysis['shiller_margin_of_safety']
...
class ValueTrapDetector:
    ...
    def check_roic_vs_wacc(stock_data: StockData) -> Dict:
        ...
        risk_free_rate = wacc_params.get('risk_free_rate', 0.05)
        market_risk_premium = wacc_params.get('market_risk_premium', 0.06)
        beta = getattr(stock_data, 'beta', None) if stock_data else None
